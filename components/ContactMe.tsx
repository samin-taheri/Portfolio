...
function ContactMe({ }: Props) {
    ...
    const onSubmit: SubmitHandler<Inputs> = (formData) => {
        window.location.href = `mailto:samintaheri1922000@gmail.com?subject=${formData.subject}&body= Hi, my name is ${formData.name}. ${formData.message} (${formData.email})`
    }
    return (
        <div className='h-screen relative flex flex-col text-center md:text-left md:flex-row max-width-7xl px-10 justify-evenly mx-auto items-center'>
            ...
            <div className='flex flex-col space-y-10'>
                <h4 className='text-3xl font-semibold text-center pt-28 md:pt-16'>I have got kust what you need.{" "}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col space-y-2 w-fit mx-auto'>
                    ...
                    <button className='bg-[#8336c8] py-4 px-10 rounded-md text-black font-bold text-lg'>Submit</button>
                </form>
            </div>
        </div>
    )
